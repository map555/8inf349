...
# Because we can init table with the old InitializeProduct function,
# we create a InitProductTable class in model.py to be able to call it in the init-db method
class InitProductTable:
    def __init__(self):
        self.__url = productURL
        self.__productsDict = self.__getProducts()
        self.__UpdateProduct()  # check if update
    ...
    def __getRequest(self):
        ...
        if code != 200:
            print("ERROR: trying to get products from \"http://jgnault.ddns.net/shops/products/\" return http code",
                  code)
            exit(0)
        return response

    def __ConvertResponseToJson(self, response):
        jsn = None
        try:
    ...
    def __getProducts(self):
        ...
        jsonDict = self.__ConvertResponseToJson(response=response)
        return jsonDict

    def __CheckExistance(self, jsonProduct):
